...
export function HabitsList({ date, onCompletedChanged }: HabitsListProps) {
    ...
    useEffect(() => {
        api.get('day', {
            params: {
                date: dayjs(date).add(2, 'hour').toISOString(), // At NLW Setup we put as date.toISOString(), however, I added 2 hours cause  of Timezone
            }
        })
            .then(response => {
    ...
    const isDateInPast = dayjs(date).endOf('day').isBefore(dayjs(new Date()));
